...
# Margin rule: top Streptococcus pneumoniae hit must beat best non-target by >= 5 bits
BIT_DELTA = 5.0

# ---- Streptococcus pneumoniae taxon matching ----
# Accept as target:
#   - Streptococcus pneumoniae (any strain name)
_SPNEUMO_PATTERNS = [
    re.compile(r'\bStreptococcus\s+pneumoniae\b', re.I),
]

def is_target_spneumo(name: str) -> bool:
    name_stripped = name.strip()
    return any(pattern.search(name_stripped) for pattern in _SPNEUMO_PATTERNS)
...
def sample_root(path: str) -> str:
    # Filenames like 1313_s.001_blast.tsv -> sample "1313_s.001"
...
def per_read_hits(tsv_path):
    ...
    with open(tsv_path, encoding="utf-8", errors="ignore") as handle:
        for line in handle:
            ...
            hits[query_id].append((bit_score, is_target_spneumo(taxon_name)))
    return hits
...
def classify_read(items):
    ...
    # Only Streptococcus pneumoniae at the top; check margin to best non-target
...
# ---- main ----
def main():
    # S. pneumoniae BLAST files start with 1313 and end with _blast.tsv
    tsv_paths = sorted(glob.glob("blast_confirm_results/1313*_blast.tsv"))
    ...
    output_name = "blast_confirm_margin_summary_spneumoniae_bitdelta5.tsv"
    with open(output_name, "w", newline="") as output_handle:
        writer = csv.writer(output_handle, delimiter="\t")
        writer.writerow([
            "sample",
            "Spneumoniae_validated_reads",
            "Spneumoniae_ambiguous_reads",
            "nonTarget_top_reads"
        ])
